...
1. Wrap `(load custom-file 'noerror 'nomessage)` — prevents fresh-clone
   init failure.
2. Rename `secrets.el` to avoid collision with built-in.
3. Fix the `gptel` API-key resolution (lambda).
4. Move `(treemacs-start-on-boot)` to `:init` so the sidebar actually
   appears.
5. Drop or fix `load-path-ignore-regexp` — it currently does nothing.
