...
(defun get-config-file (filename)
  (expand-file-name filename (concat user-emacs-directory "my_emacs/")))
#+end_src
...
;; Temporarily commented out: need to add check for emacs version
;; and only do this for versions without the user-lisp feature.
;; (add-to-list
;;  'load-path
;;  (directory-file-name
;;   (concat
...
(use-package
 ...
 (("M-/" . #'hippie-expand)
  ...
  ("C-x M-R" . (lambda () (interactive) (let ((current-prefix-arg '(4))) (call-interactively #'remember))))
  ("s-q" . nil)
 ...
 ;; emacs by default doesn't upgrade built-in packages -- causes magit to freak
...
Magit is a package to provide a git interface inside of Emacs.
...
Enables evaluation of ~typescript~ source blocks in Org via [[https://github.com/nicholasgasior/ob-ts-node][ob-ts-node]].
Requires ~ts-node~ to be available on ~exec-path~.
...
(use-package ob-ts-node
  ...
  (org-babel-do-load-languages
   ...
   (append org-babel-load-languages '((ts-node . t)))))
#+end_src
