...
**** TODO see user-lisp directory
Removed for now; if we restore it, don't change global ~default-directory~.

#+begin_src emacs-lisp
(defun add-subdirs-to-load-path (parent-directory)
  "Add PARENT-DIRECTORY and its immediate child directories to `load-path'."
  (let ((new-path-directory parent-directory))
    (add-to-list 'load-path new-path-directory)
...
Because elpaca is loading packages asynchronously, we use the :defer
keyword to delay loading until a package is used.

#+begin_src emacs-lisp
(use-package smart-mode-line :config (sml/setup) :defer)
(use-package
...
#+begin_src emacs-lisp
(use-package page-break-lines :config (global-page-break-lines-mode) :defer)
#+end_src

*** Better folding: Kirigami

#+begin_src emacs-lisp
(use-package kirigami :defer)
#+end_src
...
#+begin_src emacs-lisp
(use-package whitespace-cleanup-mode :config (global-whitespace-cleanup-mode 1) :defer)
#+end_src
...
#+begin_src emacs-lisp
(add-hook 'elpaca-after-init-hook (lambda () (treemacs-start-on-boot)))
#+end_src
...
package, but on-the-fly or local-only customizations will go to this file. It
needs to go after the use-package declarations.

#+begin_src emacs-lisp
(add-hook 'elpaca-after-init-hook (lambda ()
(load (get-config-file "custom.el"))))
#+end_src
