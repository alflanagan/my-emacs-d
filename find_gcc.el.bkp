...
;; from Robert Hallam's blog: https://blog.roberthallam.org/2026/01/fixed-error-invoking-gcc-driver-brew-macos/
;; credited to user jimmeh
...
Detects paths for gcc and libgccjit packages to be used in LIBRARY_PATH."
  (let* ((paths '())
    ...
    (when brew-bin
      ;; Get gcc paths.
      (let* ((gcc-prefix (string-trim (shell-command-to-string (concat brew-bin " --prefix gcc"))))
      ...
      ;; Get libgccjit paths
      (let* ((jit-prefix (string-trim (shell-command-to-string (concat brew-bin " --prefix libgccjit"))))
